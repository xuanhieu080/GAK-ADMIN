...
function onSubmit() {
  ...
  service.handleUpdate('create-review',`${props.id}/reviews/${props.review_id}`, reduceProperties(form, 'roles', 'id')).then(() => {
...
watch(
    ...
    async () => {
      ...
      if (props.review_id) {
        service.find(`${props.id}/reviews/${props.review_id}`).then((response) => {
          thumbImage.value = response.data.model.thumb;
          fillObject(form, response.data.model);
          form.dateTime = new Date(response.data.model.date);
          if (response.data.model.productVariant) {
            productVariant.value = response.data.model.productVariant;
          }
        })
      }
    },
