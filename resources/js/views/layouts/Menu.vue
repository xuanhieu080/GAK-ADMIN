    ...
    <ul class="space-y-2">
        <template v-for="item in $props.state.mainMenu">
                    ...
                    <template v-for="subitem in item.children">
                        <template v-if="isEnabled(subitem, $props.type)">
                            <li v-if="subitem.hasOwnProperty('onClick')">
                                <router-link :to="subitem.to ? subitem.to : '#'" @click.prevent="subitem.onClick" class="flex items-center p-2 pl-11 w-full text-base font-normal text-white font-semibold rounded-lg hover:text-theme-300 hover:bg-theme-800" :class="isActive(subitem) ? 'bg-theme-800' : ''">
                                    {{ subitem.name }}
                                    <span class="sr-only" v-html="subitem.name"></span>
                                </router-link>
                            </li>
                            <li v-else>
                                <router-link :to="subitem.to ? subitem.to : '#'" class="flex items-center p-2 pl-11 w-full text-base font-normal text-white font-semibold rounded-lg hover:text-theme-300 hover:bg-theme-800" :class="isActive(subitem) ? 'bg-theme-800' : ''">
                                    {{ subitem.name }}
                                    <span class="sr-only" v-html="subitem.name"></span>
                                </router-link>
                            </li>
            ...
            <li v-else-if="isEnabled(item, $props.type)">
                <a v-if="item.hasOwnProperty('onClick') && !item.to" href="#" class="flex items-center p-2 text-base font-normal text-white font-semibold rounded-lg hover:text-theme-300 hover:bg-theme-800" @click.prevent="item.onClick">
                    <Icon :name="item.icon" class="mr-2 text-3xl pl-2 -mt-1"/>
                    <span class="ml-1" v-html="item.name"></span>
                    <span class="sr-only" v-html="item.name"></span>
                    <span v-if="item.hasOwnProperty('label') && item.label" class="inline-flex justify-center items-center px-2 ml-3 text-sm font-medium text-gray-800 bg-gray-200 rounded-full" v-html="item.label"></span>
                </a>
                <router-link v-else :to="item.to ? item.to : '#'" class="flex items-center p-2 text-base font-normal text-white font-semibold rounded-lg hover:text-theme-300 hover:bg-theme-800" :class="isActive(item) ? 'bg-theme-800' : ''">
                    <Icon :name="item.icon" class="mr-2 text-3xl pl-2 -mt-1"/>
                    <span class="ml-1" v-html="item.name"></span>
                    <span class="sr-only" v-html="item.name"></span>
                    <span v-if="item.hasOwnProperty('label') && item.label" class="inline-flex justify-center items-center px-2 ml-3 text-sm font-medium text-gray-800 bg-gray-200 rounded-full" v-html="item.label"></span>
